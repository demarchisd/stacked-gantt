    ...
    function StackedGantt($this)
    {
      ...
      var rowValueContainerWidth;
       ...
       function sanitizeGeneralHighlightsDates(generalHighlights)
       {
        ...
        function defineLimits()
        {
           ...
           var begins = rowsLimits
        	...
        	if(begins.length) {
        	    begin = new Date(begins.reduce(function(lowestBegin, currentBegin) {
        	...
        	if(ends.length) {
        	    end = new Date(ends.reduce(function(highestEnd, currentEnd) {
        ...
        function createMarkerTooltip(marker, $marker, rowDescription)
        {
        	...
        	var squareBadge = true;
                var template = getTooltipTemplate();
                var duration = '';

        	createTooltip($marker, template, color, title, description, duration, badge, subtitle, squareBadge);
        }
        ...
        function createActivityTooltip(activity, $activity, rowDescription)
        {
        	...
        	var badge = title?title[0]:'?';
                var template = getTooltipTemplate();
                var duration = getActivityDuration(activity);

        	createTooltip($activity, template, color, title, description, duration, badge, subtitle);
        }
       ...
       function createGeneralHighlight(highlight)
       {
        ...
        {
        	if(style && style.tooltipTemplate) {
        	    return style.tooltipTemplate;
        	} else {
        	    return null;
        	}
        }
       ...
       function getHighlightColor(highlight)
       {
        ...
        function createTooltip($element, tooltiptemplate, color, title, description, duration, badge, subtitle, squareBadge)
        {
        	var handleMouseOver = function(ev)
        	{
        	    $element.addClass('hover');
                    ...
                    if(tooltiptemplate){	// there is a template, so use it
                      var items = tooltiptemplate.split(/\$\{(.+?)\}/g);
                      var para = {badge: badge, title: title, subtitle: subtitle, description: description, duration: duration};
                      var html = items.map(function(str, i){ return i % 2?para[str]:str; }).join('');
                      $tooltip = $(html);
                    } else {
        	      $tooltip = $("<div>", { class: "sg_tooltip" });
